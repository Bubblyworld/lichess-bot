...
func NegaFastEvalDelta(board *dragon.Board, move dragon.Move) EvalCp {
	myPiecePosVals := blackPiecePosVals
	yourPiecePosVals := whitePiecePosVals
	if board.Wtomove {
		myPiecePosVals = whitePiecePosVals
	...
	fromPiece := board.PieceAt(from)
	toPiece := fromPiece

	promoPiece := move.Promote()
	if promoPiece != dragon.Nothing {
		toPiece = promoPiece
	}
	...
	// En-passant
	enPassantPos := board.Enpassant()
	if to == enPassantPos && fromPiece == dragon.Pawn && enPassantPos != 0 {
		captureLocation = to + 8
		if board.Wtomove {
			captureLocation = to - 8
		}
		capturedPiece = dragon.Pawn
	}
	
	fromEval := pieceVals[fromPiece] + EvalCp(myPiecePosVals[fromPiece][from])
	toEval := pieceVals[toPiece] + EvalCp(myPiecePosVals[toPiece][to])
	captureEval := pieceVals[capturedPiece] + EvalCp(yourPiecePosVals[capturedPiece][captureLocation])

	// Check for castling - painful but shouldn't happen too often
	var castlingRookDelta EvalCp = 0
	if fromPiece == dragon.King {
		var oldRookLoc, newRookLoc uint8
		if to-from == 2 { // castle short
			oldRookLoc = to + 1
			newRookLoc = to - 1
		} else if int(to)-int(from) == -2 { // castle long
			oldRookLoc = to - 2
			newRookLoc = to + 1
		}
		myRookPosVals := myPiecePosVals[dragon.Rook]
		castlingRookDelta = EvalCp(myRookPosVals[newRookLoc] - myRookPosVals[oldRookLoc])
	}
